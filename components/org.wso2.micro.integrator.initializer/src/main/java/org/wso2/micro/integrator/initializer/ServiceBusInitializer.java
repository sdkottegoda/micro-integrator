...
*/
            ...
            if (synapseEnvironment != null) {
                ...
                if (transactionPropertyEnabled) {
                    ...
                    } catch (DataSourceUndefinedException e) {
                        log.error("Error in initializing the Transaction Count Handler component. The number of "
                                  + "transactions will not be recorded. You can ignore this error if you do not "
                                  + "want the server to keep track of the number of transactions processed", e);
                    }
                }
                if (log.isDebugEnabled()) {
